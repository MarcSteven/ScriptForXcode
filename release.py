import argparse
 import subprocess
  ...
  parser = argparse.ArgumentParser(description='Release MemoryChain Swift SDK')
 ...
 args = parser.parse_args()
  ...
  def bump_version():
     replace_version('MemoryChain-swift.podspec', args.old, args.new)
     replace_version('MemoryChain/Info.plist', args.old, args.new)
     replace_version('generate_docs.sh', args.old, args.new)
     subprocess.call('git add MemoryChain-swift.podspec', shell=True)
     subprocess.call('git add MemoryChain/Info.plist', shell=True)
